...
from llm import generate_response, clear_conversation, get_conversation_length, set_openai_key
...
async def set_openai_key_endpoint(request: APIKeyRequest):
    """Set the OpenAI API key for better LLM responses"""
    success = set_openai_key(request.api_key)
    if success:
        return {"message": "OpenAI API key set successfully. Now using GPT for responses!"}
    else:
